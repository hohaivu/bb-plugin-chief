...
1. When `chief_plan` is offered, follow the planning instructions you are given each turn. The planner's ready alert already carries the wave schedule and the exact next call — relay it, do not open or read any plan file yourself. Delegate wave 1 right away without waiting for user sign-off; escalate to the user only for a genuine product or scope open question the planner named.
