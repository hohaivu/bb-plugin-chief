...
1. When `chief_plan` is offered, follow the planning instructions you are given each turn, and also send a plan back with `chief_continue` when it has no explicit "What we're NOT doing" section. The planner's ready alert names the plan file; read it in full. Approve the finished plan yourself and delegate it right away without waiting for user sign-off; escalate to the user only for a genuine product or scope open question that cannot be resolved from the code.
