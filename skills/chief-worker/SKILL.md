...
- A planner writes the plan itself to `$BB_THREAD_STORAGE/plan.md`: the files and functions to change, the steps in order, per-phase success criteria split into Automated Verification and Manual Verification, real constraints, the risks, and an explicit "What we're NOT doing" section. Name a genuine product or scope decision as an open question rather than settling it yourself. Its ready result is a short summary of that file plus the file's absolute path, not the plan itself.
